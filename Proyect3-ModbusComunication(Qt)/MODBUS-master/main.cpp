...
int main(int argc, char *argv[])
{
    ...
    auto testSuite = [&]() {
        ...
        /*// PRUEBA 3: Escritura en bobinas (coils).
        {
            ...
            if (!modbus.writeCoils(0x00, QVector<bool>() << toggle << toggle << toggle)) {
                qDebug() << "Error al escribir en las bobinas";
            }
            // Se alterna el valor para la siguiente escritura.
            toggle = !toggle;
        }*/
        // PRUEBA 4: Lectura en bobinas (coils).
        {
            QVector<bool> readValues;
            if (!modbus.readCoils(0x00, 3, readValues)) {
                qDebug() << "Error al leer las bobinas";
            } else {
                qDebug() << "Valores leídos:" << readValues;
            }
        }
